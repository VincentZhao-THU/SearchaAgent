...
usage() {
    cat <<'EOF'
Usage:
  bash scripts/infer/run_batch_infer_existing_retriever_v04.sh --model-path <path>
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --samples-per-source) SAMPLES_PER_SOURCE_ARG="${2:-}"; shift 2 ;;
        --seed) SEED_ARG="${2:-}"; shift 2 ;;
        --topk) TOPK_ARG="${2:-}"; shift 2 ;;
        --max-new-tokens) MAX_NEW_TOKENS_ARG="${2:-}"; shift 2 ;;
        --temperature) TEMPERATURE_ARG="${2:-}"; shift 2 ;;
        --max-turns) MAX_TURNS_ARG="${2:-}"; shift 2 ;;
        --max-prompt-length) MAX_PROMPT_LENGTH_ARG="${2:-}"; shift 2 ;;
        --max-obs-length) MAX_OBS_LENGTH_ARG="${2:-}"; shift 2 ;;
        --entropy-top-k) ENTROPY_TOP_K_ARG="${2:-}"; shift 2 ;;
        --ema-alpha) EMA_ALPHA_ARG="${2:-}"; shift 2 ;;
        --trigger-threshold) TRIGGER_THRESHOLD_ARG="${2:-}"; shift 2 ;;
        --trigger-tail-k) TRIGGER_TAIL_K_ARG="${2:-}"; shift 2 ;;
        --dtype) DTYPE_ARG="${2:-}"; shift 2 ;;
        -h|--help) usage; exit 0 ;;
...
OUTPUT_DIR="${OUTPUT_DIR_ARG:-${LOG_DIR:-$ROOT_DIR/log}/batch_infer_entropy_triggered_testsets}"
...
EMA_ALPHA="${EMA_ALPHA_ARG:-0.3}"
TRIGGER_THRESHOLD="${TRIGGER_THRESHOLD_ARG:-0.2}"
TRIGGER_TAIL_K="${TRIGGER_TAIL_K_ARG:-3}"

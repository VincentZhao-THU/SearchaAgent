...
from infer_with_entropy import (  # noqa: E402
...
DEFAULT_OUTPUT_DIR = "log/batch_infer_entropy_testsets"
DEFAULT_TOPK = 3
...
def parse_args():
    parser = argparse.ArgumentParser(description="Batch inference with token entropy tracing.")
...
def main():
    ...
    log_path = output_dir / "batch_infer_entropy_testsets.log"
    jsonl_path = output_dir / "batch_infer_entropy_testsets.jsonl"
    ...
    with log_path.open("w", encoding="utf-8") as log_fp, jsonl_path.open("w", encoding="utf-8") as jsonl_fp:
        ...
        print_and_log(f"Entropy window size: {args.entropy_window_size}", log_fp)
